...
void rand_name(void)
{
	...
	sys_cfg.myname[i] = '-';
	get_mac("eth0", &(sys_cfg.myname[i + 1]));
}
...
int sys_info_default(void)
{
	...
	rand_name();
	strcpy(sys_cfg.passwd, sys_cfg.myname);
	...
	json_object_object_add(sys_info_json, "passwd", 
							json_object_new_string(sys_cfg.passwd));
...
void __get_config_for_json()
{
	...
	__json_key_to_string(sys_info_json, "passwd", sys_cfg.passwd, USER_NAME_LEN);
