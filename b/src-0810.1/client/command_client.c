...
void __send(void)
{
	char buf[8];
	__str_to_buf(buf, gp_comman_tag[1]);
	sendto_node_unreliable(buf, gp_comman_tag[2], strlen(gp_comman_tag[2]) + 1);
}
...
void ___send(void)
{
	char buf[8];
	__str_to_buf(buf, gp_comman_tag[1]);
	sendto_node_reliable(buf, gp_comman_tag[2], strlen(gp_comman_tag[2]) + 1, 10);
}
